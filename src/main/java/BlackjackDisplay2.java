...
/**
 * Servlet implementation class BlackjackDisplay2
 */
@WebServlet("/BlackjackDisplay2")
public class BlackjackDisplay2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
    public BlackjackDisplay2() {super();}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(gameState.equals("reset")){
			...
			gameboard = gameboard + cardUtils.generateButton("Begin",new int[] {5,5}, "button", "BlackjackDisplay2?choice=setup&check="+inputCheck);
		}
			...
			//out.println("<a href = \"BlackjackDisplay2?choice=reset&check="+inputCheck+"\"> RESET </a>");
		}
		...
		for(int i = 0; i < buttons.size(); i++) {
			sto = sto +
				cardUtils.generateButton(buttons.get(i), 
						new int[] {pos[0] + (ofset[0] * i) , pos[1] + (ofset[1] * i)}, "button","BlackjackDisplay2?choice="+buttons.get(i)+"&check="+inputCheck);	// link back with choice & IC update
		}
		return sto;
	}

...
@WebServlet({"/BaccaratDisplay2"})
public class BaccaratDisplay2 extends HttpServlet {
		...
		for(int i = 0; i < buttonNames.size(); i++) {
			sto = sto +
				cardUtils.generateButton(buttonNames.get(i), 
						new int[] {pos[0] + (ofset[0] * i) , pos[1] + (ofset[1] * i)}, "button","BaccaratDisplay2?choice="+buttonLinks.get(i)+"&check="+inputCheck);	// link back with choice & IC update
		}
		return sto;
	}
